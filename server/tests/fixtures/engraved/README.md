...
| `staff_size_7_0mm`, `staff_size_4_0mm`, `staff_size_2_5mm`, `staff_size_1_8mm` | the same notation-over-tablature bars engraved at four staff sizes (via `<defaults><scaling>`). Below ~3.2mm the notation and tab staves' gap shrank under a cluster window fixed in points, so the two merged into one discarded group and the page yielded nothing; both staves must now be found at every size (issue #86) |
